...
namespace GameServer.Managers
{
    class GuildManager:Singleton<GuildManager>
    {
        ...
        public void ChangeGuildNotice(string notice, Character character)
        {
            var now = DateTime.Now;
            var guild = DBService.Instance.Entities.Guilds.FirstOrDefault(c => c.Id == character.Id);
            if (guild != null)
            {
                guild.Notice = notice;
                guild.CreateTime = now;
