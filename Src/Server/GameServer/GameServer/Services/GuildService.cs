...
namespace GameServer.Services
{
    class GuildService : Singleton<GuildService>
    {
        public GuildService()
        {
            ...
            MessageDistributer<NetConnection<NetSession>>.Instance.Subscribe<GuildNoticeChangeRequest>(OnGuildNoticeChange);
        ...
        {
            Character character = sender.Session.Character;
            Log.InfoFormat("OnGuildJoinRequestL:：GuildId:{0}characterId:[{1}]{2}", request.Apply.GuildId, request.Apply.characterId, request.Apply.Name);
        ...
        private void OnGuildJoinResponse(NetConnection<NetSession> sender, GuildJoinResponse response)
        {
            ...
            if (request != null)
            {
                ...
                request.Session.Response.guildJoinRes.Errormsg = "加入公会成功";
        ...
        private void OnGuildNoticeChange(NetConnection<NetSession> sender, GuildNoticeChangeRequest request)
        {
            var character = sender.Session.Character;
            Log.InfoFormat($"OnGuildNoticeChange: : GuildNotice:{request.GuildNotice} character:[{character.Id}]:{character.Info.Name}");
            sender.Session.Response.guildNoticeChange = new GuildNoticeChangeResponse();
            if (character.Guild == null)
            {
                sender.Session.Response.guildNoticeChange.Result = Result.Failed;
                sender.Session.Response.guildNoticeChange.Errormsg = "您目前还没有公会哦！";
                sender.SendResponse();
                return;
            }
            GuildManager.Instance.ChangeGuildNotice(request.GuildNotice, character);
            sender.Session.Response.guildNoticeChange.guildInfo = character.Guild.GuildInfo(character);
            sender.Session.Response.guildNoticeChange.Result = Result.Success;
            sender.SendResponse();
        }
